...
int main ()
{
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          // Compute steer error
          // Steer error will be an angle between the closest point from the planner output trajectory (stored in x_points and y_points vectors)
          // and the actual car position (x_position, y_position)
          
          // Looking for the closest point in the planner trajectory
          ...
          // Calculating the angle between the closest point and vehicle's position
          double angle_diff = angle_between_points(x_position, y_position, x_points[min_index], y_points[min_index]);
          
          // Calculating steering CTE
          double error_steer = yaw - angle_diff;
          ...
          // Error is the difference between the actual vehicle's speed and the desired velocity 
          // of the closest point in the planner's v_points vector
          double error_throttle = velocity - v_points[min_index];
